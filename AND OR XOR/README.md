...
int main() {
    int rs1 = 0x1234; // 源暫存器rs1的值
    int rs2 = 0x5678; // 源暫存器rs2的值
    unsigned int rd; // 目標暫存器rd的值
    ...
    // OR指令
    rd = rs1 | rs2; // 執行OR指令，將rs1和rs2的值進行按位或操作，並將结果存到rd暫存器中
    std::cout << "rs1 | rs2 = 0x" << std::hex << rd << std::endl; //輸出結果

    // XOR指令
    rd = rs1 ^ rs2; // 執行XOR指令，將rs1和rs2的值進行按位或操作，並將结果存到rd暫存器中
    std::cout << "rs1 ^ rs2 = 0x" << std::hex << rd << std::endl; //輸出結果
